...
AGENTS.md 是项目级的 Agent 协作文档，记录项目特有的规则、约束、架构决策和踩坑经验。**仅在项目开发场景下维护**，日常对话、问答、非项目任务不需要创建或更新。

### 创建位置

- 放在项目根目录（与 `.git` 同级）
- 文件名：`AGENTS.md`（全大写）
- 如果项目没有 AGENTS.md，首次项目开发任务时创建

### 作用

- 告诉 Agent 项目的目录结构、技术栈、编码规范
- 记录踩过的坑和解决方案，避免重复犯错
- 定义项目特有的工作流约定（如提交规范、分支策略）

### 触发更新的时机

**前提条件**：当前任务属于项目开发（有代码仓库、有开发目标），否则跳过。

| 场景 | 操作 |
|------|------|
| 踩了同一个坑两次 | 写进 AGENTS.md |
| 用户纠正了 Agent 的工作方式 | 写进 AGENTS.md |
| 发现新的技术约束或架构决策 | 写进 AGENTS.md |
| 项目结构重大变化 | 更新 AGENTS.md |
| 完成复杂任务后（5+ 步） | 检查 AGENTS.md 是否需要补充 |

### 更新频率

- **增量更新**：每次复杂任务完成后，花 30 秒检查 AGENTS.md 是否需要补充
- **里程碑审查**：每个功能/版本完成后，通读一遍 AGENTS.md，删除过时内容

### 与 HERMES.md 的关系

- `AGENTS.md`：项目特有的规则和约定
- `HERMES.md`：superpowers-zh 生成的引导文件（工具映射、技能列表）
- 两者共存，各自负责不同层面
